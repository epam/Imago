...
namespace imago
{
	...
	bool prefilterAdaptive(Settings& vars, Image& raw)
	{
		logEnterFunction();

		Image bitmask;
		bitmask.copy(raw);
		prefilter_cv::prefilterBasic(vars, bitmask);
		...
		double lineThickness = ImageUtils::estimateLineThickness(bitmask, vars.routines.LineThick_Grid);
