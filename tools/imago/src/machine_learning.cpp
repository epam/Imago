        ...
        try
        {
            ...
            if (!progress_loaded)
            {
                ...
                for (LearningBase::iterator it = base.begin(); it != base.end(); it++)
                {
                    ...
                    {
                        printf("*** OK: %u/%u, SCORE: %g, TIME: %g ms\n", result_record.ok_count, visual_counter, result_record.average_score / visual_counter,
                               result_record.average_time / visual_counter);
                    }
                }
                        ...
                        for (int subset = 0; subset < 2; subset++)
                        {
                            ...
                            for (size_t idx = start_idx; idx < end_idx; idx++)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (quick_check && it->second.time > LEARNING_SUSPICIOUS_TIME_FACTOR * avg_time && it->second.time > LEARNING_ABNORMAL_TIME)
                                    {
