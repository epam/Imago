...
imago_root = os.path.normpath(os.path.join(os.path.abspath(__file__), '..', '..', '..', '..'))
...
def testImagoConsole(mol, image):
    ...
    subprocess.check_call([
        f"{imago_root}/dist/imago{execSuffix}",
        image,
        "-o", resultmolpath
    ])
...
if __name__ == '__main__':
    ...
    print('Successfully recognized images: %s, rate: %s ' % (
        successfullyRecognizedImages, float(successfullyRecognizedImages) / float(totalImages)))
