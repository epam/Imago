...
bool MultipleBondChecker::checkDouble(const Settings& vars, Edge frst, Edge scnd)
{
    ...
    bf = _s.getBondInfo(first);
    bs = _s.getBondInfo(second);
    ...
    if (fe != se && fb != sb)
    {
        if (_g.getEdge(fb, se).second || _g.getEdge(fe, sb).second)
            return false;
...
bool MultipleBondChecker::checkTriple(const Settings& vars, Edge thrd)
{
    ...
    double ddf = Algebra::distance2segment(fm, tb_pos, te_pos), dds = Algebra::distance2segment(sm, tb_pos, te_pos);
