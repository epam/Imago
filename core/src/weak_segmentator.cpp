...
namespace imago
{
    ...
    bool WeakSegmentator::hasRectangularStructure(const Settings& vars, int id, Rectangle& bound, int winSize)
    {
        ...
        if (get2centers(map_x, x1c, x2c) && get2centers(map_y, y1c, y2c))
        {
            ...
            // and centers
            if (get2centers(map_x, x1c, x2c) && get2centers(map_y, y1c, y2c) && fabs(x1c - x2c) > 2 * winSize && fabs(y1c - y2c) > 2 * winSize)
            {
                int good = 0, bad = 0;
                for (Points2i::iterator it = p.begin(); it != p.end(); ++it)
                    if ((fabs(it->x - x1c) < winSize || fabs(it->x - x2c) < winSize) || (fabs(it->y - y1c) < winSize || fabs(it->y - y2c) < winSize))
                        good++;
                    else
                        bad++;
