    ...
    public:
        ...
        static void putGraph(Image& img, const EuclideanGraph& cg)
        {
            ...
            for (typename EuclideanGraph::vertex_iterator begin = g.vertexBegin(), end = g.vertexEnd(); begin != end; begin = end)
                for (typename EuclideanGraph::vertex_descriptor v; begin != end ? (v = *begin, true) : false; ++begin)
                {
            ...
            for (auto begin = g.edgeBegin(), end = g.edgeEnd(); begin != end; begin = end)
                for (typename EuclideanGraph::edge_descriptor e; begin != end ? (e = *begin, true) : false; ++begin)
                {
                    Vec2d b_pos = g.getVertexPosition(e.m_source), e_pos = g.getVertexPosition(e.m_target);
