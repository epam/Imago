...
{
    Segment *firstNear = NULL, *secNear = NULL;
    ...
    if (xfirstSeparable && !yfirstSeparable && rec.height < vars.dynamic.CapitalHeight + vars.separator.ltFactor1 * vars.dynamic.LineThickness &&
        rec.height > vars.dynamic.CapitalHeight * vars.separator.capHeightMin && dist1 < vars.separator.capHeightMax * vars.dynamic.CapitalHeight &&
        vars.separator.extRatioMin > ((double)rec.width / (double)rec.height))
        return true;
    return false;
}

double HuMoms[2][2] = {{0.9419354839, 0.0682196339}, {0.0326612903, 0.8860232945}};

double Ratioclass[2][2] = {{0.9826612903, 0.6114808652}, {0.0096774194, 0.340266223}};

double KNNclass[2][2] = {{0.6596774194, 0.2978369384}, {0.3403225806, 0.7021630616}};

double Probclass[2][2] = {{0.964516129, 0.3693843594}, {0.035483871, 0.6306156406}};
...
int Separator::ClusterLines(const Settings& vars, Points2d& inputLines, IntVector& outClasses)
{
    ...
    do
    {
        ...
        for (size_t i = 0; i < inputLines.size() / 2; i++)
        {
            ...
            if (dc1 < dc2 &&
                (dist < vars.dynamic.CapitalHeight || (dist < vars.dynamic.CapitalHeight * vars.separator.getRatio2 && ratio < vars.estimation.MinSymRatio)))
                outClasses[i] = 0;
            else
                outClasses[i] = 1;
        }
...
void Separator::SeparateStuckedSymbols(const Settings& vars, SegmentDeque& layer_symbols, SegmentDeque& layer_graphics, CharacterRecognizer& rec)
{
    ...
    {
        imago::Image smallLines(vars.general.ImageWidth, vars.general.ImageHeight), largeLines(vars.general.ImageWidth, vars.general.ImageHeight);
    ...
    for (size_t i = 0; i < symbRects.size(); i++)
    {
        ...
        int right =
            round((symbRects[i].x + symbRects[i].width + line_thick > timg.getWidth()) ? timg.getWidth() : (symbRects[i].x + symbRects[i].width + line_thick));
        ...
        Image extracted(right - left + 1, bottom - top + 1), _2BClassified(right - left + 1, bottom - top + 1);
        extracted.fillWhite();
        ...
        for (SegmentDeque::iterator it = segs.begin(); it != segs.end(); ++it)
        {
            for (size_t n = 0; n < RectPoints[i].size(); n++)
            {
                ...
                if ((*it)->getX() <= dx && ((*it)->getWidth() + (*it)->getX()) > dx && (*it)->getY() <= dy && ((*it)->getHeight() + (*it)->getY()) > dy)
                {
                    int sx = dx - (*it)->getX();
                    int sy = dy - (*it)->getY();
...
int Separator::PredictGroup(const Settings& vars, Segment* seg, int mark, SegmentDeque& layer_symbols)
{
    ...
    else
    {
        ...
        aprior = 1.0 - 1.0 / (1 + std::exp(-(surfaceRatio - vars.p_estimator.LogisticLocation) / vars.p_estimator.LogisticScale));
    }
...
void Separator::ClassifySegment(const Settings& vars, SegmentDeque& layer_symbols, CharacterRecognizer& rec, Segment* seg, ClassifierResults& cresults)
{
    ...
    if (mark == SEP_SYMBOL && (!(s->getHeight() >= cap_height - sym_height_err && s->getHeight() <= cap_height + sym_height_err &&
                                 s->getHeight() <= cap_height * 2 && s->getWidth() <= cap_height) ||
    ...
    // if(mark == SEP_SUSPICIOUS || mark == SEP_BOND)
    {
        ...
        if (s->getHeight() >= cap_height - sym_height_err && s->getHeight() <= cap_height + sym_height_err && s->getHeight() <= cap_height * 2 &&
            s->getWidth() <= vars.separator.capHeightRatio2 * cap_height)
        {
            ...
            if (thinseg->getRatio() > adequate_ratio_max)
                if (ImageUtils::testSlashLine(vars, *thinseg, 0, vars.separator.testSlashLine1))
                    ...
                    mark = SEP_SPECIAL;
            else if (thinseg->getRatio() < adequate_ratio_min)
                if (_testDoubleBondV(vars, *thinseg))
                    mark = SEP_BOND;
                else
                    mark = SEP_SUSPICIOUS;
            else if (ImageUtils::testSlashLine(vars, *thinseg, 0, vars.separator.testSlashLine2))
                mark = SEP_BOND;
            else
                mark = SEP_SYMBOL;
        }
    ...
    if (segs > vars.separator.minApproxSegsStrong)
    {
        ...
        bool two_chars_probably = wh > vars.separator.extRatioMax && wh < vars.separator.ext2charRatio * vars.separator.extRatioMax;

        if (s->getHeight() > vars.separator.extCapHeightMin * cap_height && s->getHeight() < vars.separator.extCapHeightMax * cap_height &&
            wh > vars.separator.extRatioMin && (two_chars_probably || wh < vars.separator.extRatioMax))
        {
            char ch;
            ...
            else if ((two_chars_probably) || (s->getWidth() > cap_height))
            {
                ...
                if (best_x > 0)
                {
                    ...
                    if (rec.isPossibleCharacter(vars, *s1, true, &ch) && rec.isPossibleCharacter(vars, *s2, true, &ch))
                    {
    ...
    for (SegmentDeque::iterator it = _segs.begin(); it != _segs.end(); it++)
    {
        ...
        if (CharacterRecognizer::graphics.find(c) != std::string::npos && dist < vars.characters.DistanceAbsolutelySure)
        {
            ...
            getLogExt().appendText("Classified as graphics on first stage");
                 ...
                 (vars.dynamic.CapitalHeight < 0 || s->getHeight() > vars.characters.HeightMinBound * vars.dynamic.CapitalHeight &&
...
int Separator::estimateCapHeight(const Settings& vars, bool& restrictedHeight)
{
    ...
    double temp =
        StatUtils::interMean(heights.begin() + seq_pairs[symbols_found[symbols_seq]].first, heights.begin() + seq_pairs[symbols_found[symbols_seq]].second);
#ifdef DEBUG
...
bool Separator::_checkSequence(const Settings& vars, IntPair& checking, IntPair& symbols_graphics, double& density)
{
    ...
    for (int i = checking.first; i < checking.second; i++)
    {
        ...
        if (_segs[i]->getRatio() >= adequate_ratio_min && _segs[i]->getRatio() <= adequate_ratio_max)
        {
...
bool Separator::_testDoubleBondV(const Settings& vars, Segment& segment)
{
    ...
    {
        int x, y, rows = tmp.getHeight(), cols = tmp.getWidth(), y0 = segment.getY();
