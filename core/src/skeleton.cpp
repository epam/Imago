...
Skeleton::Edge Skeleton::addBond(Vertex& v1, Vertex& v2, BondType type, bool throw_if_error)
{
    ...
    Vec2d begin = _g.getVertexPosition(v1), end = _g.getVertexPosition(v2);
...
Skeleton::Edge Skeleton::addBond(const Vec2d& begin, const Vec2d& end, BondType type, bool throw_if_error)
{
...
{
    Bond f = _g.getEdgeBond(first), s = _g.getEdgeBond(second);
...
bool Skeleton::_isEqualDirection(const Vertex& b1, const Vertex& e1, const Vertex& b2, const Vertex& e2) const
{
    ...
    Vec2d begin2 = _g.getVertexPosition(b2), end2 = _g.getVertexPosition(e2);
...
bool Skeleton::_isParallel(const Edge& first, const Edge& second) const
{
    ...
    return (fabs(f.k - s.k) < _parLinesEps || fabs(fabs(f.k - s.k) - PI) < _parLinesEps);
}
...
bool Skeleton::_dissolveIntermediateVertices(const Settings& vars)
{
    ...
    for (; vi != vi_end; ++vi)
    {
        ...
        {
            dir1.diff(_g.getVertexPosition(end1), _g.getVertexPosition(beg1));
            dir2.diff(_g.getVertexPosition(end2), _g.getVertexPosition(beg2));
        }
        else if (beg1 == end2 || beg2 == end1)
        {
            dir1.diff(_g.getVertexPosition(end1), _g.getVertexPosition(beg1));
            dir2.diff(_g.getVertexPosition(beg2), _g.getVertexPosition(end2));
        }
        else
        {
...
void Skeleton::_findMultiple(const Settings& vars)
{
    ...
    do
    {
        ...
        std::sort(doubleBonds.begin(), doubleBonds.end(), DoubleBondComparator<SkeletonGraph>(getGraph()));
...
bool Skeleton::_isSegmentIntersectedByEdge(const Settings& vars, Vec2d& b, Vec2d& e, std::deque<Edge> edges)
{
    ...
    for (it = edges.begin(); it != edges.end(); ++it)
    {
        ...
        if (intersection.x < maxX + 1 && intersection.x > minX - 1 && intersection.y < maxY + 1 && intersection.y > minY - 1)
            return true;
    }
    return false;
}
...
void Skeleton::_connectBridgedBonds(const Settings& vars)
{
    ...
    for (size_t i = 0; i < edge_groups_k.size(); i++)
    {
        ...
        for (int k = 0; k < gr_count; k++)
        {
            ...
            for (int l = k + 1; l < gr_count; l++)
            {
                ...
                if (min < blockS && min > vars.skeleton.ConnectFactor * LineS && Algebra::SegmentsOnSameLine(vars, p1, p2, sp1, sp2) &&
    ...
    {
        Edge e1 = (*eit).first, e2 = (*eit).second;
        Vertex v1, v2, v3, v4;
        ...
        if (_g.getDegree(v1) > 1 || _g.getDegree(v2) > 1)
        {
            continue;
        }
