...
namespace imago
{
    ...
    std::string FunctionRecord::getPlatformSpecificInfo()
    {
        ...
        sprintf(buf, " (memory: %iKb, work time: %ims, log time: %ims, total time: %ims)", getMemDelta(), getWorkTime(), getLogTime(), getTotalTime());
        result = buf;
        return result;
    }
...
#define LOG_TIME_END                                                                                                                                           \
    for (size_t u = 0; u < Stack.size(); u++)                                                                                                                  \
    Stack[u].time_log_ms += (platform::TICKS() - log_time_start)
    ...
    void log_ext::enterFunction(const std::string& name)
    {
        ...
        dump(getStringPrefix(true) + "<div title=\"" + fr.name + "\" style=\"background-color: " + color + ";\" >" +
             "<b><font size=\"+1\">Enter into <a href=\"#" + fr.anchor + "\">" + fr.name + "</a> function</font></b><div style=\"margin-left: 20px;\">");
        Stack.push_back(fr);
    }
    ...
    void log_ext::leaveFunction()
    {
        ...
        dump(getStringPrefix() + "</div><b><font size=\"+1\">Leave from <a name=\"" + fr.anchor + "\">" + fr.name + "</a> function</font></b>" +
             fr.getPlatformSpecificInfo() + " </div>");
    }
