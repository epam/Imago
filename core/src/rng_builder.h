...
namespace imago
{
    ...
    public:
        ...
        static void build(EuclideanGraph& g)
        {
            ...
            int i = 0;
            for (typename EuclideanGraph::vertex_iterator begin = g.vertexBegin(), end = g.vertexEnd(); begin != end; ++begin)
            {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    distances[i + j * n] = Vec2d::distance(g.getVertexPosition(ind2vert[i]), g.getVertexPosition(ind2vert[j]));
                }
            }
                ...
                for (int j = 0; j < n; j++)
                {
                    if (i != j)
                    {
                        ...
                        for (int k = 0; k < n; k++)
                        {
                            if (k != i && k != j)
                            {
                                if (d > std::max(distances[i + k * n], distances[k + j * n]))
                                {
                                    add_edge = false;
                                    break;
                                }
