...
bool testNear(const Vec2d& point, const Rectangle& rec, double margin)
{
    ...
    return point.x < (rec.x + rec.width + margin) && point.x > left && point.y < (rec.y + rec.height + margin) && point.y > top;
}
...
bool testCollision(const Vec2d& beg, const Vec2d& end, const Rectangle& rec)
{
    ...
    Vec2d vec1(rec.x, rec.y), vec2(rec.x + rec.width, rec.y), vec3(rec.x, rec.y + rec.height), vec4(rec.x + rec.width, rec.y + rec.height);
    vec1.diff(vec1, end);
    vec2.diff(vec2, end);
    vec3.diff(vec3, end);
    vec4.diff(vec4, end);
    ...
    double min = std::min<double>(a1, std::min<double>(a2, std::min(a3, a4)));
    double max = std::max<double>(a1, std::max<double>(a2, std::max(a3, a4)));
    return min * max < 0;
}
