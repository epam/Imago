...
{
    size_t dfb = _g.getDegree(fb), dfe = _g.getDegree(fe), dsb = _g.getDegree(sb), dse = _g.getDegree(se);
    ...
    else
    {
        ...
        Vertex nb = _s.addVertex(p1), ne = _s.addVertex(p2);
...
DoubleBondMaker::Result DoubleBondMaker::operator()(const std::pair<Edge, Edge>& edges)
{
    ...
    if (!_g.getEdge(fb, fe).second || !_g.getEdge(sb, se).second)
        return std::make_tuple(0, empty, empty);
    ...
    double dbb = Vec2d::distance(fb_pos, sb_pos) + Vec2d::distance(fe_pos, se_pos), dbe = Vec2d::distance(fb_pos, se_pos) + Vec2d::distance(fe_pos, sb_pos);
    ...
    if (_g.getDegree(fb) < _g.getDegree(fe) && _g.getDegree(sb) < _g.getDegree(se))
    {
        std::swap(fb, fe);
    ...
    double fl = bf.length, sl = bs.length;
    double mult;
