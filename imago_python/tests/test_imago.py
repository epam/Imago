...
from tests import OUTPUT_DIR, CAFFEINE_JPG
...
class ImagoTest(unittest.TestCase):
    ...
    def test_load_pillow_save_pillow(self) -> None:
        ...
        original_path = OUTPUT_DIR / 'test_load_pillow_save_pillow_original.jpg'
    ...
    def test_load_pillow_save_file(self) -> None:
        ...
        self.imago.save_image_to_file(OUTPUT_DIR / 'test_load_pillow_save_file.jpg')

    def test_load_file_save_file(self) -> None:
        self.imago.load_image_from_file(CAFFEINE_JPG)
        self.imago.save_image_to_file(OUTPUT_DIR / 'test_load_file_save_file.jpg')

    def test_load_buffer_save_file(self) -> None:
        with CAFFEINE_JPG.open('rb') as f:
            data = f.read()
        self.imago.load_image_from_buffer(data)
        self.imago.save_image_to_file(OUTPUT_DIR / 'test_load_buffer_save_file.jpg')
    ...
    def test_all_to_file(self) -> None:
        self.imago.load_image_from_file(CAFFEINE_JPG)
        self.imago.filter_image(ImagoFilter.BASIC)
        warnings = self.imago.recognize()  # TODO: check warnings if there count should be 0 (currently 2)
    ...
    def test_all_to_str(self) -> None:
        self.imago.load_image_from_file(CAFFEINE_JPG)
        self.imago.filter_image(ImagoFilter.BASIC)
        warnings = self.imago.recognize()  # TODO: find out why we have 2 warnings, but no log records, is it ok?
